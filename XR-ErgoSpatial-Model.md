...
当前主流 XR 设备（如 Apple Vision Pro）同时支持两种交互模态：**直接触摸（Direct Touch）** 和 **眼动+捏合（Gaze & Pinch）**。后者允许用户在手臂自然下垂的状态下，通过注视目标并捏合手指完成操作，几乎不产生上肢疲劳。一个自然的问题随之而来：**既然眼动+捏合可以零疲劳地覆盖全空间，为什么还需要关注直接触摸及其带来的手臂负荷？**

原因在于，两种模态的能力边界存在本质差异，直接触摸在以下维度上不可被眼动+捏合替代：

**输入带宽：** 直接触摸是一个 6 自由度的连续空间输入——手在空间中的位置和朝向每一帧都在提供信息。而眼动+捏合的结构是"注视提供 2D 方向 + 捏合提供二值确认"，输入带宽远低于直接触摸。这意味着滑块拖拽、3D 物体旋转、绘图等需要连续精细空间控制的操作，眼动+捏合的表达能力不足。

**双手交互：** 眼动在任意时刻只有一个注视焦点，天然是单通道的。而直接触摸允许双手同时操作不同对象（如左手固定模型、右手旋转），这对空间内容编辑类应用是刚需。

**意图明确性：** 眼睛的自然行为是不断扫视——人每秒产生 3–4 次扫视，绝大多数注视不携带交互意图。将注视用作瞄准通道时，系统必须消歧"看了一眼"与"想操作它"（即 Midas Touch 问题），要么引入停留时间延迟，要么增加额外确认步骤。而伸手触碰是一个需要主动发起的动作，意图信号天然明确。

**空间认知与临场感：** 当手直接触碰虚拟物体时，用户通过本体感觉（proprioception）建立"这个东西在我身体前方这个位置"的空间心理模型。眼动+捏合将操作抽象化，手不需要移动到目标位置，空间关系退化为纯视觉信息，用户对界面布局的空间记忆会减弱。更重要的是，XR 区别于传统屏幕的核心价值之一就是具身交互——如果所有操作都退化为"看一眼+捏一下"，XR 交互在体验上就退化成了戴在头上的鼠标，失去了媒介独有的直觉性。

因此，直接触摸和眼动+捏合是互补关系，而非替代关系：

| 维度 | 直接触摸 | 眼动+捏合 |
| :--- | :--- | :--- |
| 最优距离 | 近场（可达范围内） | 远场（任意距离） |
| 输入带宽 | 高（6DOF 连续） | 低（2D 方向 + 二值确认） |
| 适合的操作 | 精细控制、拖拽、双手操作 | 快速选择、确认、滚动 |
| 疲劳代价 | 取决于手臂姿态 | 极低（手臂无需抬起） |
| 意图明确性 | 高（主动伸手） | 需消歧（Midas Touch） |
| 空间认知 | 强（本体感觉参与） | 弱（纯视觉） |

既然直接触摸在 XR 交互中不可或缺，那么它带来的手臂疲劳就是一个必须被系统性管理的问题——这正是本文档存在的前提。
...
产品上线后，每位用户的身体数据会通过 2.2 的校准流程实时采集，界面布局会据此个性化适配。但在设计阶段，设计师需要在没有真实用户的情况下搭建原型并验证布局。此时需要带入参考体型数据来驱动模型。

**主参考：设计师本人的身体数据**

设计师本人是原型阶段的第一个测试者。使用设计师自己的身体数据驱动模型，可以让模型的输出与设计师的第一人称体感完全对齐——模型显示舒适区的位置，设计师伸手过去确实感到舒适；模型显示高负荷的位置，设计师也确实感到吃力。这种一致性是建立对模型信任、高效迭代原型的基础。

**边界校验：5th 百分位女性与 95th 百分位男性**

设计师自己的体型只是人群中的一个点。为确保布局对目标用户群体普遍适用，需要用两组边界数据进行额外验证，它们分别从两个方向约束设计：

| 边界数据 | 验证方向 | 约束对象 | 验证的问题 |
| :--- | :--- | :--- | :--- |
| 5th 百分位女性（小体型） | 向内收缩（最小可用范围） | 界面布局 | 核心组件能否在最小体型用户的舒适区内容纳？ |
| 95th 百分位男性（大体型） | 向外扩张（最大空间占用） | 物理环境 | 使用空间是否足够容纳最大体型用户的交互包络？ |

- **5th 百分位女性**拥有最短的臂长和最小的舒适区。如果所有高频交互组件在这个最小舒适区内都能放下并正常使用，更大体型的用户必然也能舒适使用。这为界面布局设定了硬约束。
- **95th 百分位男性**拥有最长的臂长和最大的交互包络。当这个用户伸展手臂执行操作时，动作范围远大于小体型用户。这为物理使用环境设定了净空要求——墙壁、家具、其他人不能侵入该包络，否则会阻碍交互或造成碰撞。

**参考数值：**

| 参数 | 5th 百分位女性 | 50th 百分位（中间值） | 95th 百分位男性 |
| :--- | :--- | :--- | :--- |
| 全臂长（肩到腕） | ~0.46 m | ~0.56 m | ~0.66 m |
| 上臂长（× 0.55） | ~0.253 m | ~0.308 m | ~0.363 m |
| 前臂长（× 0.45） | ~0.207 m | ~0.252 m | ~0.297 m |
| 肩关节垂直偏移 | ~0.19 m | ~0.22 m | ~0.25 m |
| 肩关节水平偏移 | ~0.15 m | ~0.18 m | ~0.21 m |
...
**前提假设：手臂无外部支撑。** 本模型假设手臂在空间中自由悬空，肩部肌群独自对抗手臂全部重力。这对应当前空间计算设备（如 Apple Vision Pro）的典型使用场景——用户坐在沙发/椅子上或站立，手臂在空间中自由移动，没有稳定的支撑面。

如果手臂存在外部支撑（如肘部搁在桌面、前臂平放在扶手上），支撑面会提供向上的法向力，直接抵消部分甚至全部重力载荷，肩关节力矩相应降低。此时舒适区会显著扩张，过渡区和高负荷区相应收缩——极端情况下（前臂完全平放在桌面），桌面平面内的操作几乎不产生肩关节力矩，负荷水平接近传统桌面 GUI。当前模型未纳入支撑力建模，因为支撑面的空间位置、接触点和接触方式在现有传感器条件下不易获取。在有明确支撑面的使用场景中，本模型的疲劳评估会偏保守（高估实际负荷）。
