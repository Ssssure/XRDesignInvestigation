...
**关于操作时长限制与 MVC 耐受时长的关系：** 4.5 节引用的 MVC 文献给出的是一次性静态维持某负荷水平直到疲劳出现的**生理耐受时长**（过渡区约数分钟，高负荷区约数十秒至数分钟）。而此处的操作时长限制是面向整个使用会话的**设计指导值**，两者度量的尺度不同。

XR 应用的典型会话时长可达 30 分钟甚至更久，在此期间用户可能反复进入较高负荷区域执行操作。如果每次停留都逼近生理耐受极限，累积疲劳会远超单次测量所暗示的水平，且肌肉在两次操作之间可能无法充分恢复。因此，操作时长限制在生理耐受时长的基础上施加了大幅余量，确保单次操作足够短暂，使手臂在操作间隙能回到低负荷或静息状态，从而将整个会话的累积疲劳控制在可接受范围内。
...
**高负荷区推荐动作（直接触摸模态，但建议优先切换为眼动+手势）：**

| 动作类型 | 适用理由 |
| :--- | :--- |
| 单次点按 (Tap) | 最简单的触发动作，接触即松手，停留时间最短 |
| 大面积按压 (Large Target Press) | 放大目标尺寸以降低瞄准精度要求，弥补手臂不稳定带来的误触风险 |

> 该区域虽然物理上可达，但手臂负荷高、稳定性差。如果交互频率不是极低，应优先使用眼动+捏合等远场模态替代直接触摸。
...
在空间中均匀采样大量点，为每个点计算疲劳指数，生成一个 3D 热力图。设计师可以直观看到哪里是绿色（舒适）、哪里是红色（高负荷），从而在设计阶段就建立对空间负荷分布的全局认知，指导后续的布局决策。
...
### 5.5 设计流程指南：从组件清单到空间布局

当设计师已经确认了产品的功能列表和所需的交互组件，可以按以下流程使用本文档将它们落位到 XR 空间中。

**第一步：组件盘点与特征标注**

列出所有需要放置的组件，为每个组件标注三个交互特征：

| 特征 | 问题 | 示例 |
| :--- | :--- | :--- |
| 交互频率 | 用户在一次会话中大约操作多少次？ | 键盘：数百次；设置开关：1–2 次 |
| 单次操作时长 | 每次操作需要手停留多久？ | 滑块拖拽：3–10 秒；按钮点按：<1 秒 |
| 精度需求 | 操作是否要求手部稳定或精细控制？ | 绘图：高精度；通知消除：低精度 |

这一步的目的是让每个组件拥有可与疲劳分区匹配的量化特征，而非凭直觉分配位置。

**第二步：将组件分配到疲劳分区**

根据第一步标注的特征，对照 4.7（通用功能级推荐）和 4.8（具体组件级推荐）的分区标准，将每个组件归入目标分区：
...
如果某个组件的特征跨越两个分区（例如中等频率、中等精度），优先归入负荷更低的分区——宁可浪费舒适空间，也不要让用户在高负荷位置执行超出该区域承受能力的操作。

**第三步：生成热力图，建立空间全局认知**

使用 5.1 的可视化热力图功能，输入目标用户群体的身体数据（或使用第 50 百分位参考体型），生成 3D 疲劳分布图。此步骤帮助设计师直观理解：

- 舒适区、过渡区、高负荷区在空间中的实际形状和范围
- 各分区的边界是弯曲的三维曲面，而非简单的矩形平面
- 左右手的可达范围和舒适区可能不对称
...
从第二步的分区结果中，优先处理归入舒适区的核心组件（它们占据最宝贵的空间资源）。使用 5.2 的最优位置推荐功能，逐个查询每个核心组件的推荐放置坐标。

需要注意的是，多个组件不能占据同一位置。当推荐位置发生重叠时，按交互频率从高到低排优先级——频率最高的组件优先占据最优位置，其余组件向周边偏移。

**第五步：手动布局辅助组件，用逐点查询验证**

核心组件落位后，在热力图上为过渡区和高负荷区的组件手动选择候选位置。布局时遵循以下原则：

- 过渡区组件尽量靠近舒适区边界，缩短手臂从舒适区移动过去的距离
- 高负荷区组件放大目标尺寸，降低瞄准精度要求
- 不可达区内容无需考虑手部触达，专注视觉可读性

对每个候选位置使用 5.3 的逐点查询功能验证，确认实际疲劳指数落在预期分区内。如果某个位置的疲劳指数超出目标分区范围，调整位置后重新查询。

**第六步：为每个位置指定交互模态与动作类型**

组件位置确定后，根据 4.9（交互动作类型推荐）为每个组件指定具体的交互方式：

- 舒适区内的组件 → 直接触摸，可使用精细拖拽、双手操作等复杂动作
- 过渡区内的组件 → 直接触摸，限于单次点按、挥动等简单动作
- 高负荷区内的组件 → 优先使用眼动+捏合替代直接触摸
- 不可达区内的内容 → 必须使用眼动+捏合或射线

**第七步：用户测试与迭代**

以上流程产出的是基于生物力学模型的初始布局方案。将方案实装后进行用户测试，重点观察：

- 用户是否在过渡区或高负荷区出现频繁操作（说明组件分区可能不当）
- 用户是否反馈某些舒适区组件"够不太舒服"（说明个体差异超出模型预期，需调整）
- 实际操作时长是否超出 4.7 中的建议限制

根据测试结果微调组件位置和分区边界阈值，重复第五、六步。
